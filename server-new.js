...
async function initializeData() {
  try {
    ...
    // ALWAYS ensure Bella exists (for existing databases)
    ...
    if (!bellaExists) {
      ...
      await bella.save();
      console.log('✅ Bella creator account added to existing database');
    } else if (!bellaExists.isActive) {
      bellaExists.isActive = true;
      await bellaExists.save();
      console.log('✅ Bella creator account activated');
    } else {
      console.log('✅ Bella creator account already exists and is active');
    }
...
// MIGRATION: Fix old FanPurchase records to use correct CreatorAccount IDs
app.post('/api/migrate-creator-accounts', authenticateToken, requireManager, async (req, res) => {
  try {
    console.log('🔄 Starting creator account migration...');
    
    // Get all creator accounts
    const accounts = await CreatorAccount.find({});
    console.log('📋 CreatorAccounts found:', accounts.map(a => a.name));
    ...
    console.log('🗺️ Name to ID map:', Object.keys(nameToIdMap));
    ...
    for (const report of reports) {
      ...
      if (result.modifiedCount > 0) {
        ...
        results.push({
          ...
          purchasesUpdated: result.modifiedCount
        });
        console.log(`✅ Updated ${result.modifiedCount} purchases for ${report.creator} (${report.date.toISOString().split('T')[0]})`);
      }
    }
    ...
    for (const group of purchasesByModel) {
      ...
      distribution[account ? account.name : 'Unknown'] = group.count;
    }
...
app.get('/api/analytics/dashboard', checkDatabaseConnection, authenticateToken, async (req, res) => {
  try {
    ...
    // FIXED: Use ONLY ChatterPerformance data for unlock rate (most reliable - manually uploaded from platform)
    // This avoids double-counting and uses the actual sent/unlocked data you upload
    const combinedPPVsUnlocked = chatterPPVsUnlocked; // Use chatter performance data only
